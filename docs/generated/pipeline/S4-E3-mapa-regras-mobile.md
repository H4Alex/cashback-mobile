# S4-E3 — Mapa de Regras de Negócio: Consumer MOBILE
...
> Alterações rastreáveis: [S4-E1 Diff Mobile #N] por item
...
| 5 | ~~Divider "ou"~~ | ~~View~~ | **REMOVIDO** [S4-E1 Diff Mobile #1] | — | — |
| 6 | ~~Botão "Entrar com Google"~~ | ~~TouchableOpacity~~ | **REMOVIDO** [S4-E1 Diff Mobile #1] | — | — |
| 7 | ~~Botão "Entrar com Apple"~~ | ~~TouchableOpacity~~ | **REMOVIDO** [S4-E1 Diff Mobile #1] | — | — |
...
| ~~2~~ | ~~OAuth Google~~ | ~~`POST /api/mobile/v1/auth/oauth`~~ | — | — | — | **REMOVIDO** [S4-E1 Diff Mobile #1] |
| ~~3~~ | ~~OAuth Apple~~ | ~~`POST /api/mobile/v1/auth/oauth`~~ | — | — | — | **REMOVIDO** [S4-E1 Diff Mobile #1] |
...
| ~~OAuth placeholder~~ | — | **REMOVIDO** [S4-E1 Diff Mobile #1] | — | — |
...
|----------|--------|-------|--------|
| ~~Botão Apple Sign-In~~ | **REMOVIDO** junto com todos os botões OAuth [S4-E1 Diff Mobile #1] | — | — |
...
| email | `z.string().email()` | "E-mail inválido" |
| cpf | `z.string().length(11).refine(isValidCPF)` | "CPF inválido" (Mod-11 algorithm) [S4-E1 Diff Mobile #2] |
| senha | `z.string().min(6)` | "Senha deve ter no mínimo 6 caracteres" |
...
| 3 | Campo Código | TextInput | step === "verify" | step !== "verify" | Vazio [S4-E1 Diff Mobile #3 — step renomeado] |
| 4 | Botão "Verificar Código" | TouchableOpacity | step === "verify" | step !== "verify" | — [S4-E1 Diff Mobile #3 — NOVO step intermediário] |
| 5 | Campo Nova Senha | TextInput secureTextEntry | step === "reset" | step !== "reset" | Vazio [S4-E1 Diff Mobile #3] |
| 6 | Botão "Redefinir Senha" | TouchableOpacity | step === "reset" | step !== "reset" | — |
| 7 | Tela de Sucesso | View | step === "done" | step !== "done" | — [S4-E1 Diff Mobile #3] |
| 8 | Link "Voltar ao login" | TouchableOpacity | Sempre (exceto done) | — | — |

### Regras de Interação
| # | Ação | Endpoint | Método | Payload | Validações | Feedback |
|---|------|----------|--------|---------|------------|----------|
| 1 | Enviar email | `POST /api/mobile/v1/auth/forgot-password` | POST | `{ email }` | Zod: email válido | Sucesso → step "verify"; Erro → Alert "Não foi possível enviar..." [S4-E1 Diff Mobile #3] |
| 2 | Verificar token (NOVO) | `POST /api/mobile/v1/auth/verify-reset-token` | POST | `{ email, token }` | Zod: email, token min 1 | Sucesso → step "reset" (response: `{valid, expires_in}`); Erro 400/410 → Alert [S4-E1 Diff Mobile #3 — NOVO endpoint] |
| 3 | Redefinir senha | `POST /api/mobile/v1/auth/reset-password` | POST | `{ email, token, senha }` | Zod: email, token min 1, senha min 6 | Sucesso → step "done" + Alert "Senha redefinida!"; Erro → Alert "Token inválido ou expirado" [S4-E1 Diff Mobile #3] |

### Dependências entre Campos
| Campo Controlador | Valor | Campo Afetado | Comportamento | Regra Backend |
|-------------------|-------|---------------|---------------|---------------|
| step | "email" → "verify" → "reset" → "done" | Formulário visível | Fluxo em 4 steps; email copiado entre forms automaticamente [S4-E1 Diff Mobile #3] | — |
...
| 4 | Tap em transação | — | — | — | status ∈ CONTESTABLE_STATUSES (`rejeitado`, `expirado`, `congelado`) [S4-E1 Diff Mobile #4] | router.push contestacao/create com cashback_entry_id e empresa_nome |
...
| Endpoint | Método | Campos Utilizados | Dependências |
|----------|--------|-------------------|--------------|
| `GET /api/mobile/v1/saldo` | GET | saldo_total, por_empresa (empresa_id, nome_fantasia, saldo, logo_url), proximo_a_expirar: `{valor, quantidade}` | useSaldo() [S4-E1 Diff Mobile #5 — RESOLVIDO: schema corrigido de number para object] |
...
| Endpoint | Método | Campos Utilizados | Dependências |
|----------|--------|-------------------|--------------|
| `GET /api/mobile/v1/notifications/preferences` | GET | push_enabled, email_enabled, marketing_promocoes | useNotificationPreferences() [S4-E1 Diff Mobile #6 — response simplificado para 3 booleans flat] |
...
| 3 | Toggle "Push Notifications" | Switch | Sempre | — | prefs.push_enabled [S4-E1 Diff Mobile #6] |
| 4 | Toggle "Email" | Switch | Sempre | — | prefs.email_enabled [S4-E1 Diff Mobile #6] |
| 5 | Toggle "Marketing e Promoções" | Switch | Sempre | — | prefs.marketing_promocoes [S4-E1 Diff Mobile #6 — consolidado de 3 toggles para 1] |
| ~~6~~ | ~~Toggle "Transações"~~ | — | **REMOVIDO** [S4-E1 Diff Mobile #6] | — | — |
| ~~7~~ | ~~Toggle "Promoções"~~ | — | **REMOVIDO** [S4-E1 Diff Mobile #6] | — | — |
| ~~8~~ | ~~Toggle "Marketing"~~ | — | **REMOVIDO** [S4-E1 Diff Mobile #6] | — | — |
...
| 5 | Toggle Biometria OFF | `POST /api/mobile/v1/auth/biometric/unenroll` | POST | `{ device_id }` | biometricEnrolled === true | Chama API unenroll + setBiometricEnrolled(false) [S4-E1 Diff Mobile #7 — NOVO endpoint] |
...
| 3 | Campo "Confirmar Nova Senha" | TextInput secureTextEntry | Sempre | — | Vazio [S4-E1 Diff Mobile #8 — NOVO campo] |
...
| nova_senha_confirmation | `z.string().min(6).refine(nova_senha === nova_senha_confirmation)` | "Senhas não conferem" [S4-E1 Diff Mobile #8] |
...
| 2 | Confirmar exclusão | `DELETE /api/mobile/v1/auth/delete-account` | DELETE [S4-E1 Diff Mobile #9] | `{ senha, motivo? }` | — | Sucesso → Alert "Conta Excluída" → router.replace login; Erro → Alert "Não foi possível excluir a conta. Verifique sua senha." |
...
| Regra # | Arquivo | Método | Evidência |
|---------|---------|--------|-----------|
| 1-2 | src/services/mobile.auth.service.ts:91 | deleteAccount() | DELETE /auth/delete-account + clearTokens [S4-E1 Diff Mobile #9] |
...
| 2 | FilterChips (status) | Componente | Sempre | — | Nenhum selecionado [S4-E1 Diff Mobile #10 — NOVO: filtro client-side] |
| 3 | Botão "Nova" | TouchableOpacity bg-blue-600 | Sempre | — | — |
| 4 | Lista ContestacaoRow | FlatList | items > 0 | items === 0 | — |
...
| `GET /api/v1/dashboard/chart` | GET | data, gerado, utilizado (série temporal) | useDashboardChart(chartPeriod) [S4-E1 Diff Mobile #11 — período dinâmico] |
...
| 3 | PeriodChips (7d/30d/90d) | Componente | Sempre | — | "7d" selecionado [S4-E1 Diff Mobile #11 — NOVO] |
| 4 | DashboardChart | Componente | chartData carregado | loadingChart | Período dinâmico via `chartPeriod` state [S4-E1 Diff Mobile #11] |
...
| 3 | Botão "Simular Scan" | TouchableOpacity | step === "scan" **AND `__DEV__ === true`** | Produção / step !== "scan" | — [S4-E1 Diff Mobile #14] |
...
| # | Elemento | Tipo | Exibir quando | Ocultar quando | Padrão |
|---|----------|------|---------------|----------------|--------|
| 1 | Menu "Campanhas" | TouchableOpacity | perfil ∈ {proprietario, gestor} | operador, vendedor | — [S4-E1 Diff Mobile #17] |
| 2 | Menu "Vendas" | TouchableOpacity | perfil ∈ {proprietario, gestor, operador} | vendedor | — [S4-E1 Diff Mobile #17] |
| 3 | Menu "Contestações" | TouchableOpacity | perfil ∈ {proprietario, gestor} | operador, vendedor | — [S4-E1 Diff Mobile #17] |
| 4 | Menu "Relatórios" | TouchableOpacity | perfil ∈ {proprietario, gestor} | operador, vendedor | — [S4-E1 Diff Mobile #17] |
| 5 | Menu "Configurações" | TouchableOpacity | perfil === proprietario | gestor, operador, vendedor | — [S4-E1 Diff Mobile #17] |
...
| Filtro | Opções | Padrão | Tipo |
|--------|--------|--------|------|
| status | ativa, inativa, finalizada | Nenhum (todas) | FilterChips [S4-E1 Diff Mobile #15] |
...
| finalizada | bg-red-100 | text-red-600 | [S4-E1 Diff Mobile #15] |
...
## Cross-Cutting Changes (S3/S4) [S4-E1 Diff Mobile — 9 itens]

| # | Item | Categoria | Detalhes |
|---|------|-----------|---------|
| 1 | Dual schema consolidação | REGRA CORRIGIDA | Re-export shim: `src/schemas/` re-exporta de `src/contracts/schemas/` (canonical) |
| 2 | Contract violation system | REGRA NOVA | `apiCall<T>()` wrapper com `schema.safeParse()` + graceful degradation |
| 3 | Cursor pagination schema | REGRA NOVA | `cursorPaginationMetaSchema`: `{next_cursor, prev_cursor, per_page, has_more_pages}` para extrato/historico/notifications |
| 4 | Campanha status enum | REGRA CORRIGIDA | `encerrada` → `finalizada` em todos os consumers |
| 5 | Token key alignment | INFERIDO→RESOLVIDO | `token` (não `access_token`) — alinhado com backend real |
| 6 | Biometric unenroll endpoint | REGRA NOVA | `POST /api/mobile/v1/auth/biometric/unenroll` com `{device_id}` |
| 7 | Push device unregister | REGRA NOVA (parcial) | Função `unregisterDevice()` disponível mas **não integrada no logout** |
| 8 | TransactionCard colors | REGRA CORRIGIDA | Negativos: `text-gray-400` → `text-red-500` |
| 9 | Notification config format | INFERIDO→RESOLVIDO | Backend: flat booleans; Frontend: transforma `{canal,ativo}` → `{[canal]:ativo}` |

### Role-Based Menu Gating [S4-E1 Diff Mobile #17 — RESOLVIDO]
...
| Relatórios | ✅ | ✅ | ❌ | ❌ |
| Configurações | ✅ | ❌ | ❌ | ❌ |
| Trocar empresa | ✅ (se multiloja) | ✅ (se multiloja) | ✅ (se multiloja) | ✅ (se multiloja) |
...
| `perfil: proprietario` | Perfil do merchant na empresa | Acesso total a todos os menus [S4-E1 Diff Mobile #17 — RESOLVIDO] |
| `perfil: gestor` | Perfil do merchant na empresa | Campanhas, Vendas, Contestações, Relatórios [S4-E1 Diff Mobile #17 — RESOLVIDO] |
| `perfil: operador` | Perfil do merchant na empresa | Apenas Vendas [S4-E1 Diff Mobile #17 — RESOLVIDO] |
| `perfil: vendedor` | Perfil do merchant na empresa | Nenhum menu extra (apenas tabs Cashback, Clientes) [S4-E1 Diff Mobile #17 — RESOLVIDO] |
...
| estornado | Estornado | FilterChip (orange) | Estado final [S4-E1 — novo status forward-compatible] |
...
| finalizada | Finalizadas | bg-red-100 text-red-600 | Estado final [S4-E1 Diff Mobile #15] |
